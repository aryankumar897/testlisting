...
export default function AgentChat() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(`${process.env.API}/admin/test`);
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try {
        const url = `${process.env.API}/admin/conversation?receiver_id=${activeUser._id}${ activeListing ? `&listing_id=${activeListing._id}` : "" }`;
  ...
  const markMessagesAsRead = async (userId) => {
    try {
      const response = await fetch(`${process.env.API}/admin/markasread`, {
        method: "POST",
  ...
  const sendTypingIndicator = async (isTyping) => {
    ...
    try {
      await fetch(`${process.env.API}/admin/typing`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.API}/admin/postmsg`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
