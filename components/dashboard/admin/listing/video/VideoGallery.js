...
const ProductGallery = () => {
  ...
  const fetchGallery = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${process.env.API}/admin/listing-video?listing_id=${id}`);
  ...
  const handleVideoUpload = async () => {
    ...
    try {
      ...
      const saveRes = await fetch(`${process.env.API}/admin/listing-video`, {
        method: "POST",
  ...
  const handleDelete = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${process.env.API}/admin/listing-video/${itemToDelete}`, {
        method: "DELETE",
      });
